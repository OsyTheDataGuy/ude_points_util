...
`find_most_similar_past_opponents(df, fighter_name, future_opponent_name, exclude_future_opponent=True, as_of=None, min_columns=None)` — for a fighter's upcoming opponent, ranks their own past opponents by similarity to that opponent. **Physical** (age, height, reach) and **style** similarity are reported separately, not blended into one score — a fighter can be a close physical match while fighting a completely different style. Style is 6 columns: `dynamic_*` striking/TD **accuracy & defense** (rate-based — silent on volume) plus, since 2026-09-11, `dynamic_sig_strikes_attempt_rate`/`dynamic_td_attempt_rate` (cumulative attempts per minute / per 15 minutes — added because accuracy alone can't tell a low-volume counter-striker from a high-output pressure fighter, or a wrestler who rarely shoots from one who shoots constantly; both existing accuracy columns are silent on frequency). Every compared column is min-max scaled before differencing (unscaled differences let whichever column has the largest raw numeric range dominate the ranking). `total_difference` is the mean of the *available* `|diff|` values per row; a column with no discriminating range contributes `NaN`, not `0.0` (a `0.0` reads as a perfect match — the neutral-state-to-boundary bug class). Because mean-of-available grades a data-poor candidate on an easier subset, each row also carries `n_columns_compared`, and any row below `min_columns` (default: all columns for physical, 5 of 6 for style) gets a `NaN` `total_difference` and sorts last rather than winning on a thin subset. `as_of` restricts every input — the future opponent's profile and the past-opponent set — to fights strictly before that date; without it a retrospective call silently uses the future opponent's stats as they stand today. `stance_match` (`'same'`/`'different'`/`'unknown'`) and `fight_date` are attached alongside the score, not folded into the distance. `exclude_future_opponent=True` (default) drops the future opponent's own past meeting(s) with the fighter from their own comparison set — without it, a fighter who's already fought their upcoming opponent trivially ranks that real fight as the "most similar" match to itself.
