...
`pdi_margin` (the single input driving `dominance_adjustment`, `perf_scale`, and the method×PDI residual, all at once) is built in `calculate_phase_magnitude_and_pdi` from five phase magnitudes: striking, control time, takedowns, submission attempts, knockdowns. Takedown and submission magnitude use `_dominance_magnitude`, a smooth blend (soft-AND of two sigmoids, over landed-count and proportion-of-total-landed) between a "close" curve (capped 0.35) and a "decisive" curve (0.36+). Landed counts are integers, so the count-axis width (`count_width=0.10`) is deliberately tight: low counts (1-3 landed) stay within ~0.004 of the fully-capped value, keeping strong small-sample protection; the smoothing itself comes from the proportion axis (genuinely continuous) and the continuous transition through 0.35–0.36.
...
**Why shrunk per-fight rate, not raw cumulative career points:** `career_point_gain` (cumulative UDE points earned) rewards volume as much as quality — on `current_df.csv`, Dustin Poirier (`fighter_url=029eaff01e6bb8f0`, 32 fights) ranks #31 by raw cumulative total but only #56 by shrunk rate, because a below-average per-fight rate (1.06, vs. Georges St-Pierre's (`fighter_url=6506c1d34da9c013`) 7.04) compounds into a large total purely by fighting more times. Dividing by `n_fights` (`raw_rate`) fixes that but creates the opposite problem at the other end: with n=1, Frank Shamrock (`fighter_url=fcaae0385b514f11`) and Bas Rutten (`fighter_url=03688dc3c3af3ac1`) post `raw_rate` ≈ 5.0–5.1 (higher than St-Pierre's career rate) off a single fight each — a sample far too small to trust as a "true" rate. Bayesian shrinkage resolves both at once: `shrunk_rate` is a weighted average of a fighter's own `raw_rate` and the full population's `population_mean_rate` (currently ≈ −1.30, dragged negative because 39.5% of all 2,555 scored fighters have 3 or fewer fights and average −2.43 — most short UFC careers end on a loss, not a win), weighted by `n_fights` vs. `prior_strength` (10.0, in equivalent-fights units). A 1-fight career gets pulled almost entirely to the population mean (Shamrock: 5.10 → −0.72); a 22-fight career like St-Pierre's barely moves (7.04 → 4.43) because real evidence outweighs the prior 2-to-1. Shrinkage alone still isn't a complete fix at the margin, though — Shavkat Rakhmonov's (`fighter_url=01afe0916a40c7c5`) 7-fight, `shrunk_rate`-only ranking (unfiltered) would place him #37 overall, ahead of many 15+ fight veterans — which is exactly why `min_fights=10` exists as a hard floor on top of the shrinkage, rather than relying on the prior alone to argue every small sample back into line.
...
**Scope and statistic:** physical measures barely differ by division (1.25–1.53× spread); `dynamic_kd_rate` differs 7.35× (heavyweight power variance dwarfs bantamweight's) and the grappling-usage columns differ ~2×. Division-scoping costs nothing where it doesn't matter and fixes real distortion where it does, rather than special-casing which columns get it. Physical columns are clean (|skew| < 0.3, <1% outliers), but most style columns are not (`dynamic_kd_rate` skew 13.3; several usage columns skew 3–6) — a robust (median/MAD) scale applied *uniformly* gives the needed outlier resistance without a mixed-footing average across columns; costs nothing on the clean columns (age's plain SD, 4.14, and its robust estimate, 4.21, are functionally identical). MAD is exactly 0 in 4 division×column cells (`dynamic_kd_rate` in three lower-power divisions, `dynamic_sub_att_rate` in HW, where most fighters share the same near-zero rate) — falls back to that column's promotion-wide scale rather than dividing by zero.

Scores are comparable across different calls: removing one unrelated candidate from a real candidate set changes every other score by exactly 0.00%. A 2-past-opponent career, Aaron Rosa (`fighter_url=11f8860028b68fcf`), produces real, non-trivial scaled differences (0.61, 0.97), not degenerate 0/±1 values.
...
- **`grappling_control_and_finishing`** (8 cols) — `dynamic_td_accuracy`/`_defence`, `dynamic_ctrl_time_share`, `dynamic_sub_att_rate`, `dynamic_ground_strikes_accuracy`/`_defence`, `dynamic_ground_strikes_per_control_minute`, `dynamic_sub_attempts_per_control_minute`. Ground-strike quality sits here, not in `striking_placement` — "what a fighter does once it's on the ground" (takedown entries, control, strikes, submissions) is one story, and ground-strike output specifically is what separates real ground-and-pound (high control time AND high ground-strike accuracy/output) from a wrestler who controls without striking much once there. `dynamic_ctrl_time_share` is a fraction of total cage time (bounded [0,1]), not a per-time rate like the others in this axis — control time is itself bounded by the fight's own duration, so it's framed as share, not frequency. The two `_per_control_minute` columns answer a different question again from either share or the total-fight-time rate columns: "how busy is this fighter once they actually have someone controlled." Khabib Nurmagomedov (`fighter_url=032cc3922d871c7f`) and Jailton Almeida (`fighter_url=41e83a89929d1327`) have a similar total-fight-time `dynamic_ground_strikes_attempt_rate` (3.08 vs. 2.79) despite Almeida's much higher `dynamic_ground_strikes_share` (0.80 vs. 0.38), but Khabib's `dynamic_ground_strikes_per_control_minute` (4.96) is ~46% higher than Almeida's (3.40) — Khabib is the higher-output ground striker once on top; Almeida holds control longer (higher `dynamic_ctrl_time_share`) and converts more of it into finish-hunting (higher `dynamic_sub_att_rate`) than volume striking. **Bayesian-shrunk, not a raw ratio** — `_shrink_rate` (the same mechanism potency/power/durability already use) toward the population's own pooled rate, prior strength 30 control-minutes (~3x the population median). Alex Pereira (`fighter_url=e5549c82bfb5582d`) has full 27/27 style coverage. Caveat that remains, not fixable from the columns available: the numerator still isn't strictly "while in control" — a strike or submission attempt can happen from the bottom or mid-scramble, since UFCStats' box score doesn't separate the two. Full detail in `data_dictionary.md`.

The axes separate seven face-validity fighters along their real reputations — Israel Adesanya (`fighter_url=1338e2c7480bdf9e`), Michael Page (`fighter_url=a67d071163962af8`), Petr Yan (`fighter_url=d661ce4da776fc20`), Merab Dvalishvili (`fighter_url=c03520b5c88ed6b4`), Khabib Nurmagomedov (`fighter_url=032cc3922d871c7f`), Jailton Almeida (`fighter_url=41e83a89929d1327`), Alex Pereira (`fighter_url=e5549c82bfb5582d`): Almeida is the extreme outlier on ground share (0.80) and control share (0.79); Dvalishvili leads every fighter on output pace (10.2 strikes/min, 16.9 TD attempts/15min) while posting the lowest power reading (0.001); Nurmagomedov and Almeida both show high ground-strike accuracy (0.73, 0.72) alongside their high control shares, distinguishing genuine ground-and-pound from a stalling profile that would show high control with low ground-strike output; Pereira and Adesanya are the two highest on power (0.022, 0.021) with Pereira lowest on grappling engagement. One data-scope limitation, not a metric defect: Page reads `dynamic_kd_rate = 0.000` despite a real highlight-reel knockout reputation — his 6 fights in this (UFC-only) dataset are all recent (2024–2026) and none ended in a knockdown; his finishing reputation is almost entirely pre-UFC (Bellator) and isn't in this data at all.

### 4f. Fighter archetype classification

`classify_fighter_archetype(df, fighter_name, as_of=None)` — two composite scores from `generate_fighter_profile`'s style snapshot, same division-scoped robust-z machinery as 4e.

`grappling_orientation` = mean(mean(z(ground_share), z(ctrl_time_share)), z(td_accuracy), z(td_attempt_rate)). Hierarchical: `ground_share`/`ctrl_time_share` correlate (r=0.69), averaged first to avoid double-counting.

`ground_game_style` = z(ground_strikes_per_control_minute) − z(sub_attempts_per_control_minute). Positive = volume ground striker; negative = finish-hunter; near zero = balanced.

`archetype_label` — hierarchical tercile buckets (30th/70th percentile, from 793 fighters with ≥8 career fights; thin careers understate `ground_game_style`'s true spread via shrinkage, so excluded). orientation ≤ −0.243 → "primarily a striker" (style ignored). orientation ≥ 0.372 → grappler, then by style: ≤ −0.912 "control/submission-leaning grappler", ≥ 1.273 "ground-and-pound-leaning grappler", else "balanced on the ground". Else "well-rounded". `None` when orientation is `NaN`. Thresholds hardcoded (`ARCHETYPE_*_PCT30`/`_PCT70`), frozen — won't self-update as the dataset grows.

Two diagnostics ride along: `ground_strikes_per_control_minute_max_fight_share`, `sub_attempts_per_control_minute_max_fight_share` — largest single fight's share of that rate's career numerator; `NaN` below 5 career events (share is meaningless below that). Flags a rate built on one outlier performance, not a sustained pattern — e.g. Petr Yan's (`fighter_url=d661ce4da776fc20`) ground-strike rate is 41% one fight, his TKO of Jose Aldo.

### 4g. Style-matchup effect on fight outcome — inconclusive, not pursued further

Tested whether a style mismatch between two fighters (via 4f's `grappling_orientation`/`ground_game_style`) predicts fight outcome beyond each fighter's existing quality level, on 1,030 fights (2015+, both fighters ≥8 prior fights at fight time; logistic regression, `quality_gap` control, interaction terms, cluster-bootstrapped by fighter).

`ground_game_style`: no effect. Interaction term p=0.937; bootstrapped win-probability swing at a realistic ±1.0 SD contrast is ~0pp (95% CI ±2.6pp) — rules out anything ≥10pp.

`grappling_orientation`: inconclusive, not resolvable with more of the same kind of data. Interaction term is marginal (p=0.075) but sizing it into a concrete win-probability swing requires two fighters simultaneously at a style extreme — real UFC matchmaking essentially never pairs two maximally-similar or maximally-opposite grapplers against each other (0 of 1,030 fights have both fighters at `|grappling_orientation| >= 1.5`; only 4 of 1,030 clear 1.0). That's a property of who actually gets matched to fight whom, not a sample-size problem a bigger dataset would fix.

Not a `ude_points_algorithm.py` v3 candidate as currently scoped.
...
The weekly refresh (`fighter_scrape_new.py` → `ude_scrape_new.py` → ETL → scoring) runs automated via GitHub Actions, in the project's repo (`OsyTheDataGuy/ude_points_util`). Live as of 31 August 2026.
