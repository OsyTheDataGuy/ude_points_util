...
`find_most_similar_past_opponents(df, fighter_name, future_opponent_name, exclude_future_opponent=True, as_of=None, min_columns=None)` — for a fighter's upcoming opponent, ranks their own past opponents by similarity to that opponent. **Physical** (age, height, reach) and **style** similarity are reported separately, not blended into one score — a fighter can be a close physical match while fighting a completely different style. Every compared column is min-max scaled before differencing (unscaled differences let whichever column has the largest raw numeric range dominate the ranking). `total_difference` is the mean of the *available* `|diff|` values per row; a column with no discriminating range contributes `NaN`, not `0.0` (a `0.0` reads as a perfect match — the neutral-state-to-boundary bug class). Because mean-of-available grades a data-poor candidate on an easier subset, each row also carries `n_columns_compared`, and any row below `min_columns` (default: all columns for physical, `len − 1` for style — 24 of 25) gets a `NaN` `total_difference` and sorts last rather than winning on a thin subset; checked directly against a real call (Makhachev/Della Maddalena) rather than assumed to still hold at the larger style-column count — 16 of 18 past opponents clear the floor, the 2 that don't are opponents with no prior cumulative data at all (a UFC debut), which is the intended behavior, not the floor being too strict. `as_of` restricts every input — the future opponent's profile and the past-opponent set — to fights strictly before that date; without it a retrospective call silently uses the future opponent's stats as they stand today. `stance_match` (`'same'`/`'different'`/`'unknown'`) and `fight_date` are attached alongside the score, not folded into the distance. `exclude_future_opponent=True` (default) drops the future opponent's own past meeting(s) with the fighter from their own comparison set — without it, a fighter who's already fought their upcoming opponent trivially ranks that real fight as the "most similar" match to itself.
